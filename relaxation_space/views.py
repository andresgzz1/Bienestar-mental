...
@login_required()
def adminView_rp_sounds(request, idsound):
    user = request.user
    if user.is_authenticated:
        if user.is_admin:
            soundlist=[]
            
            if sp.objects.filter(id=idsound).exists():
                sound= sp.objects.get(id=idsound)
                if sound_space.objects.filter(sound=sound).exists():
                    sounds = sound_space.objects.filter(sound=sound)
                    soundlist.extend(sounds)
                else:
                    soundlist=[]
                return render(request, 'admin/sound_relax_space.html', {'sound': soundlist, 'space': sound})
            else:
                messages.add_message(request=request, level=messages.ERROR, message="No se ha encontrado espacios de relajación")
                return redirect('login2')
        else:
...
@login_required()
def add_sound_rp(request, idsound):
    user = request.user
    if user.is_authenticated():
        if user.is_admin:
            if sp.objects.filter(id=idsound).exists():
                space_object = sp.objects.get(id=idsound)
                name_music = request.POST.get('txtNameSound')
                sound_space = request.FILES.get('sound')


                gif_space.objects.create(
                        name_sound = name_music,
                        sound_space = sound_space,
                        space = space_object,
                    )
                messages.add_message(request=request, level = messages.SUCCESS, message="Sonido agregada correctamente al espacio de relajación " + space_object.space_name + ".", extra_tags='alert alert-success alert-dismissible fade show')
                    
                return redirect('adminView_rp_sound', idsound)
            else:
                messages.add_message(request=request, level=messages.ERROR, message="No se ha encontrado espacios de relajación")
                return redirect('login2')
        else:
    ...
    if user.is_authenticated:
        if user.is_admin:
            if sound_space.objects.filter(id=idsound).exists:
               sound_space.objects.get(id=idsound).delete()
            messages.add_message(request=request, level=messages.SUCCESS, message="Sound deleted successfully")
            return redirect('adminView_rp')
        else:
            return render(request, 'admin/admin_relax_space_add.html')
    else:
